...
/* enums */
enum { CurNormal, CurResize, CurResizeHorzArrow, CurResizeVertArrow, CurMove, CurLast}; /* cursor */
...
killclient(const Arg *arg)
{
	if (!selmon->sel)
		return;
	if (!sendevent(selmon->sel->win, wmatom[WMDelete], NoEventMask, wmatom[WMDelete],
	      CurrentTime, 0 , 0, 0)) {
		XGrabServer(dpy);
...
setfocus(Client *c)
{
  ...
  sendevent(c->win, wmatom[WMTakeFocus], NoEventMask, wmatom[WMTakeFocus], CurrentTime,
      0, 0, 0);
}
